...
async function main() {
  ...
  const mapping = {}
  let counter = 0
  for (const [timbre, data] of Object.entries(manifest)) {
    mapping[timbre] = {}
    for (const [note, info] of Object.entries(data.samples)) {
      counter += 1
      const identifier = `s${counter}`
      imports.push(`import ${identifier} from './${timbre}/${info.file}'`)
      mapping[timbre][note] = identifier
    }
  }
  const generated = [
    ...
    ...Object.entries(mapping).flatMap(([timbre, notes]) => [
      `  ${JSON.stringify(timbre)}: {`,
      ...Object.entries(notes).map(([note, identifier]) => `    ${JSON.stringify(note)}: ${identifier},`),
      '  },',
    ]),
    '}',
    '',
  ].join('\n')
  await writeFile(path.join(OUT, 'generated.ts'), generated)
  console.log(`generated.ts escrito (${counter} imports estáticos)`)
  await rm(TMP, { recursive: true, force: true })
